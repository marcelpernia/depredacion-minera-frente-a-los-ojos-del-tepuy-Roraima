...
map.on('click', 'mining-inside-canaima', function(e) {
  ...
  var hectare = (size > 2) ? ' hectáreas' : ' hectárea'; // Return plural for measure unit name
  var size_no_spanish = size.toString().replace(".", ","); // Use comma instead of dot for decimal separators
  ...
  popupmining_inside.setLngLat([e.lngLat.lng, e.lngLat.lat])
    .setHTML('<h2>' + name + '</h2>' + '<p><strong>Área minera: </strong>' + mining_area + '</p>' + '<p><strong>Tamaño del foco: </strong>' + size_no_spanish + hectare + '<p><strong>Año detectado: </strong>' + year + '<p><strong>Fuente: </strong>' + source + '</p>')
    .addTo(map);
});
...
map.on('click', 'mining-outside-canaima', function(e) {
  ...
  // Populate the popup and set its coordinates based on the feature found.
  popupmining_inside.setLngLat([e.lngLat.lng, e.lngLat.lat])
    .setHTML('<h2>FOCO DE MINERÍA</h2>' + '<p><strong>Tamaño del foco: </strong>' + size_no_spanish + hectare + '<p><strong>Año detectado: </strong>' + year + '<p><strong>Fuente: </strong>' + source + '</p>')
    .addTo(map);
});
